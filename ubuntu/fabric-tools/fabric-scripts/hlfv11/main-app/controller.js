...
return{
	get_all_machine: function(req, res){
		console.log("getting all machine from database: ");
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    // queryAllmachine - requires no arguments , ex: args: [''],
		    const request = {
	...
	add_machine: function(req, res){
		console.log("submit recording of a machine: ");

		var array = req.params.machine.split("-");
		console.log(array);

		var key = array[0]
		var timestamp = array[2]
		var toolwear = array[1]
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    const request = {
		        ...
		        chaincodeId: 'main-app',
		        fcn: 'recordMachine',
		        args: [key, toolwear, timestamp],
		        chainId: 'composerchannel',
		        txId: tx_id
		    };
	...
	get_machine: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    if (query_responses && query_responses.length == 1) {
		        if (query_responses[0] instanceof Error) {
		            console.error("error from query = ", query_responses[0]);
		            res.send("Could not locate machine")
	...
	change_toolwear: function(req, res){
		console.log("changing toolwear of machine: ");

		var array = req.params.toolwear.split("-");
		var key = array[0]
		var toolwear = array[1];
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    if (user_from_store && user_from_store.isEnrolled()) {
				...
				console.log('im heerreeeee!!');

		    // get a transaction id object based on the current user assigned to fabric client
		    tx_id = fabric_client.newTransactionID();
		    console.log("Assigning transaction_id: ", tx_id._transaction_id);

		    // changeToolwear - requires 2 args , ex: args: ['1', 'Barry'],
		    // send proposal to endorser
		    var request = {
		        //targets : --- letting this default to the peers assigned to the channel
		        chaincodeId: 'main-app',
		    ...
		    if (isProposalGood) {
		        ...
		        console.error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
		        res.send("Error: no machine catch found");
		        // throw new Error('Failed to send Proposal or receive valid response. Response null or status is not 200. exiting...');
		    }
		}).then((results) => {
		    console.log('Send transaction promise and event listener promise have completed');
		    ...
		    console.error('Failed to invoke successfully :: ' + err);
		    res.send("Error: no machine found");
		});

	},
	get_toolwear: function(req, res){
		console.log("getting toolwear of machine: ");
